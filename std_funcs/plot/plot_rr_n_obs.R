plot_rr_n_obs <- function(results, n_obs) {
  ggplot(results, aes(x = alpha, y = mean_rejection_rate, color = statistic, fill = statistic)) +
    ...
    labs(title = paste("Rejection Rates by Alpha with SE Bands,", n_obs, "observations" ), x = "Significance Level (alpha)", y = "Rejection Rate") +
    theme_minimal()
}
