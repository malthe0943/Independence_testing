plot_rr_n_perm <- function(results, n_perm) {
  ggplot(results, aes(x = alpha, y = mean_rejection_rate, color = statistic, fill = statistic)) +
    ...
    labs(title = paste("Rejection Rates by Alpha with SE Bands,", n_perm, "permutations" ), x = "Significance Level (alpha)", y = "Rejection Rate") +
    theme_minimal()
}
